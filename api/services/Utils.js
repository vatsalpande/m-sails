...
module.exports = {
  ...
  select: function(data, attrs) {
    var col, cpRec, rec, results, _i, _j, _len, _len1;
    results = [];
  ...
  getParamState: function(req, defaultData) {
    ...
    state = _.assign(state, defaultData, function(oldV, newV) {
      if (!oldV) {
        return newV;
      } else {
        return oldV;
  ...
  getWithCache: function(cacheCollection, key, func, expire) {
    var defer;
    defer = Promise.defer();
    Cache.get(cacheCollection, key).then(function(ret) {
      if (ret.length > 0) {
